...
HEADER="\\documentclass[language=english,open=any]{njurepo}\n\\begin{document}\n\\frontmatter\n\\input{parts/examples/cover}\n"
FOOTER="\n\\end{document}"
...
def parse():
    ...
    parser.add_argument("-g", "--generate", help="generate main.tex\n option: essay, single")
            ...
            if args.name==None:
                ...
                if (args.generate=="essay"):
                    with open("main.tex", "w") as f:
                        f.write(HEADER.replace(EXAMPLE,args.name)+ESSAY.replace(EXAMPLE,args.name)+FOOTER)
